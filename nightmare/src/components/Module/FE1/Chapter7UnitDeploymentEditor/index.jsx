...
import {
  FE1Chapter7UnitDeploymentEditorEntries,
} from './options';
...
export default function FE1Chapter7UnitDeploymentEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x20855;
  const size = 29;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 7 Unit Deployment Editor by Darrman"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE1Chapter7UnitDeploymentEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE1Chapter7UnitDeploymentEditorEntries}
        />
      </div>
      <InputDec
        type={DataType.U8}
