...
import {
  FE1Chapter11UnitDeploymentEditorEntries,
} from './options';
...
export default function FE1Chapter11UnitDeploymentEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x208D3;
  const size = 27;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 11 Unit Deployment Editor by Darrman"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE1Chapter11UnitDeploymentEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE1Chapter11UnitDeploymentEditorEntries}
        />
      </div>
      <InputDec
        type={DataType.U8}
