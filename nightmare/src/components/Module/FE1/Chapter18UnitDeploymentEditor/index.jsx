...
import {
  FE1Chapter18UnitDeploymentEditorEntries,
} from './options';
...
export default function FE1Chapter18UnitDeploymentEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x209b8;
  const size = 31;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 18 Unit Deployment Editor by Darrman"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE1Chapter18UnitDeploymentEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE1Chapter18UnitDeploymentEditorEntries}
        />
      </div>
      <InputDec
        type={DataType.U8}
