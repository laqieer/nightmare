...
import {
  FE1Chapter4UnitDeploymentEditorEntries,
} from './options';
...
export default function FE1Chapter4UnitDeploymentEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x207fe;
  const size = 29;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 4 Unit Deployment Editor by Darrman"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE1Chapter4UnitDeploymentEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE1Chapter4UnitDeploymentEditorEntries}
        />
      </div>
      <InputDec
        type={DataType.U8}
