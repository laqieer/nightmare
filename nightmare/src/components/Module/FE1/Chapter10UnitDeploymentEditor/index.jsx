...
import {
  FE1Chapter10UnitDeploymentEditorEntries,
} from './options';
...
export default function FE1Chapter10UnitDeploymentEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x208B2;
  const size = 27;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 10 Unit Deployment Editor by Darrman"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE1Chapter10UnitDeploymentEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE1Chapter10UnitDeploymentEditorEntries}
        />
      </div>
      <InputDec
        type={DataType.U8}
