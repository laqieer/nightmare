...
import {
  FE1Chapter15UnitDeploymentEditorEntries,
} from './options';
...
export default function FE1Chapter15UnitDeploymentEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x20955;
  const size = 31;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 15 Unit Deployment Editor by Darrman"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE1Chapter15UnitDeploymentEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE1Chapter15UnitDeploymentEditorEntries}
        />
      </div>
      <InputDec
        type={DataType.U8}
