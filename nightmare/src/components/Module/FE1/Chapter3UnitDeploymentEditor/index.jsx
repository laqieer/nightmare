...
import {
  FE1Chapter3UnitDeploymentEditorEntries,
} from './options';
...
export default function FE1Chapter3UnitDeploymentEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x207e3;
  const size = 29;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 3 Unit Deployment Editor by Darrman"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE1Chapter3UnitDeploymentEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE1Chapter3UnitDeploymentEditorEntries}
        />
      </div>
      <InputDec
        type={DataType.U8}
