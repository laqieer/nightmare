...
import {
  FE1Chapter9UnitDeploymentEditorEntries,
} from './options';
...
export default function FE1Chapter9UnitDeploymentEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x20891;
  const size = 29;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 9 Unit Deployment Editor by Darrman"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE1Chapter9UnitDeploymentEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE1Chapter9UnitDeploymentEditorEntries}
        />
      </div>
      <InputDec
        type={DataType.U8}
