...
import {
  FE1Chapter16UnitDeploymentEditorEntries,
} from './options';
...
export default function FE1Chapter16UnitDeploymentEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x20976;
  const size = 31;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 16 Unit Deployment Editor by Darrman"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE1Chapter16UnitDeploymentEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE1Chapter16UnitDeploymentEditorEntries}
        />
      </div>
      <InputDec
        type={DataType.U8}
