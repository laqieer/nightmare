...
import {
  FE1Chapter6UnitDeploymentEditorEntries,
} from './options';
...
export default function FE1Chapter6UnitDeploymentEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x20834;
  const size = 29;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 6 Unit Deployment Editor by Darrman"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE1Chapter6UnitDeploymentEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE1Chapter6UnitDeploymentEditorEntries}
        />
      </div>
      <InputDec
        type={DataType.U8}
