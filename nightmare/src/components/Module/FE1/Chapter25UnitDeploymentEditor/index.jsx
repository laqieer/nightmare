...
import {
  FE1Chapter25UnitDeploymentEditorEntries,
} from './options';
...
export default function FE1Chapter25UnitDeploymentEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x20a91;
  const size = 31;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 25 Unit Deployment Editor by Darrman"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE1Chapter25UnitDeploymentEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE1Chapter25UnitDeploymentEditorEntries}
        />
      </div>
      <InputDec
        type={DataType.U8}
