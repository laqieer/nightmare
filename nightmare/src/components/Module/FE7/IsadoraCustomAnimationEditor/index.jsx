...
export default function FE7IsadoraCustomAnimationEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xC9960C;
  const size = 28;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Isadora Custom Animation Editor by Arch"
        type="info"
      />
      <div key={id}>
        <InputSelect
      ...
      <InputDropbox
        ...
        options={WeaponAnimationType}
        view={view}
        name="+19 All weapons or a certain one?"
        offset={25}
      />
      <InputDropbox
        isHex
