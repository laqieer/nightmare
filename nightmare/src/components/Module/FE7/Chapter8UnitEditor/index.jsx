...
import {
  FE7Chapter8UnitEditorEntries, CharacterList, ClassList, StartingLevel,
...
export default function FE7Chapter8UnitEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xCC70F4;
  const size = 16;
  let view = null;
  ...
  return (
    ...
    >
      ...
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE7Chapter8UnitEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE7Chapter8UnitEditorEntries}
        />
      </div>
      <InputDropbox
        isHex
