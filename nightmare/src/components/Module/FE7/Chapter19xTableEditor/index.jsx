...
import {
  FE7Chapter19xTableEditorEntries,
} from './options';
import { DataType } from '../../../util';
import InputSelect from '../../../Input/InputSelect';
import InputHex from '../../../Input/InputHex';

export default function FE7Chapter19xTableEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xca2ef8;
  const size = 64;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 19x Table Editor by Fire Blazer/Keriku- Credit to Icy Toast"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE7Chapter19xTableEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE7Chapter19xTableEditorEntries}
        />
      </div>
      <InputHex
        type={DataType.U32}
