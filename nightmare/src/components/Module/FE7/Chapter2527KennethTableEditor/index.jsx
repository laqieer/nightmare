...
import {
  FE7Chapter2527KennethTableEditorEntries,
} from './options';
import { DataType } from '../../../util';
import InputSelect from '../../../Input/InputSelect';
import InputHex from '../../../Input/InputHex';

export default function FE7Chapter2527KennethTableEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xca4d2c;
  const size = 64;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 25-27 Kenneth Table Editor by Fire Blazer/Keriku"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE7Chapter2527KennethTableEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE7Chapter2527KennethTableEditorEntries}
        />
      </div>
      <InputHex
        type={DataType.U32}
