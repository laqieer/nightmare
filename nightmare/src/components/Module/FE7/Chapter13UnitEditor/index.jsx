...
import {
  FE7Chapter13UnitEditorEntries, CharacterList, ClassList, StartingLevel,
...
export default function FE7Chapter13UnitEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xCC86D4;
  const size = 16;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 13 Unit Editor"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE7Chapter13UnitEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE7Chapter13UnitEditorEntries}
        />
      </div>
      <InputDropbox
        isHex
