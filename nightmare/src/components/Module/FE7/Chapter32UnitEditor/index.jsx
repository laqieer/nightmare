...
import {
  FE7Chapter32UnitEditorEntries, CharacterList, ClassList, StartingLevel,
...
export default function FE7Chapter32UnitEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xCDDED0;
  const size = 16;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 32 Unit Editor"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE7Chapter32UnitEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE7Chapter32UnitEditorEntries}
        />
      </div>
      <InputDropbox
        isHex
