...
import {
  FE7Chapter1UnitEditorEntries, CharacterList, ClassList, StartingLevel,
...
export default function FE7Chapter1UnitEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xCC5BD0;
  const size = 16;
  let view = null;
  ...
  return (
    ...
    >
      ...
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE7Chapter1UnitEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE7Chapter1UnitEditorEntries}
        />
      </div>
      <InputDropbox
        isHex
