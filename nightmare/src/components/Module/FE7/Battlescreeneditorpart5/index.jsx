...
import {
  FE7Battlescreeneditorpart5Entries, Block, PaletteFlip,
} from './options';
import { DataType } from '../../../util';
import InputSelect from '../../../Input/InputSelect';
import InputDropbox from '../../../Input/InputDropbox';

export default function FE7Battlescreeneditorpart5() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x1D8F50;
  const size = 80;
  let view = null;
  ...
  return (
    ...
    >
      ...
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE7Battlescreeneditorpart5Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE7Battlescreeneditorpart5Entries}
        />
      </div>
      <InputDropbox
        isHex
