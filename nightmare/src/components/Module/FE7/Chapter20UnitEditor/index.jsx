...
import {
  FE7Chapter20UnitEditorEntries, CharacterList, ClassList, StartingLevel,
...
export default function FE7Chapter20UnitEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xCCECEC;
  const size = 16;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 20 Unit Editor"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE7Chapter20UnitEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE7Chapter20UnitEditorEntries}
        />
      </div>
      <InputDropbox
        isHex
