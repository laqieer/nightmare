...
import {
  FE7Chapter1TableEditorEntries,
} from './options';
import { DataType } from '../../../util';
import InputSelect from '../../../Input/InputSelect';
import InputHex from '../../../Input/InputHex';

export default function FE7Chapter1TableEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xca0974;
  const size = 64;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 1 Table Editor by Fire Blazer/Keriku- Credit to Icy Toast"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE7Chapter1TableEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE7Chapter1TableEditorEntries}
        />
      </div>
      <InputHex
        type={DataType.U32}
