...
import {
  FE7Chapter31UnitEditorEntries, CharacterList, ClassList, StartingLevel,
...
export default function FE7Chapter31UnitEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xCDC87C;
  const size = 16;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 31 Unit Editor"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE7Chapter31UnitEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE7Chapter31UnitEditorEntries}
        />
      </div>
      <InputDropbox
        isHex
