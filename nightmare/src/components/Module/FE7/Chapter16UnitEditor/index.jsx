...
import {
  FE7Chapter16UnitEditorEntries, CharacterList, ClassList, StartingLevel,
...
export default function FE7Chapter16UnitEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xCCA198;
  const size = 16;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 16 Unit Editor"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE7Chapter16UnitEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE7Chapter16UnitEditorEntries}
        />
      </div>
      <InputDropbox
        isHex
