...
import {
  FE8Battlescreeneditorpart5Entries, Block, PaletteFlip,
} from './options';
import { DataType } from '../../../util';
import InputSelect from '../../../Input/InputSelect';
import InputDropbox from '../../../Input/InputDropbox';

export default function FE8Battlescreeneditorpart5() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x802508;
  const size = 80;
  let view = null;
  ...
  return (
    ...
    >
      ...
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE8Battlescreeneditorpart5Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE8Battlescreeneditorpart5Entries}
        />
      </div>
      <InputDropbox
        isHex
