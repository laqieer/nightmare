...
import {
  FE8PrologueUnitEditorEntries, CharacterList, ClassHexList, StartingLevel,
...
export default function FE8PrologueUnitEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x8B3C14;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Prologue Unit Editor"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE8PrologueUnitEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE8PrologueUnitEditorEntries}
        />
      </div>
      <InputDropbox
        isHex
