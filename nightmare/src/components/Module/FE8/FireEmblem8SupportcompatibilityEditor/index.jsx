...
import {
  SupportCharacterList, CharacterList,
} from './options';
...
export default function FE8FireEmblem8SupportcompatibilityEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x8AEBAC;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Fire Emblem 8 Support compatibility Editor (By Vennobennu with thanks to Icy Toast)"
        type="info"
      />
      <div key={id}>
        <InputSelect
