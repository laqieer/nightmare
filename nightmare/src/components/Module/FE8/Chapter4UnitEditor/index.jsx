...
import {
  FE8Chapter4UnitEditorEntries, CharacterList, ClassHexList, StartingLevel,
...
export default function FE8Chapter4UnitEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x8B4904;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 4 Unit Editor"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE8Chapter4UnitEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE8Chapter4UnitEditorEntries}
        />
      </div>
      <InputDropbox
        isHex
