...
import {
  FE8Chapter2UnitEditorEntries, CharacterList, ClassHexList, StartingLevel,
...
export default function FE8Chapter2UnitEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x8B42CC;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 2 Unit Editor"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE8Chapter2UnitEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE8Chapter2UnitEditorEntries}
        />
      </div>
      <InputDropbox
        isHex
