...
import {
  FE8Chapter6UnitEditorEntries, CharacterList, ClassHexList, StartingLevel,
...
export default function FE8Chapter6UnitEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x8B61A8;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Chapter 6 Unit Editor"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE8Chapter6UnitEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE8Chapter6UnitEditorEntries}
        />
      </div>
      <InputDropbox
        isHex
