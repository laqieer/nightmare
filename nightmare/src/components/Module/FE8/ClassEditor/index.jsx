...
import {
  FE8ClassEditorEntries, ClassList, WalkingSpeed, PortraitList,
  Ability1, Ability2, Ability3, Ability4,
  WeaponRank, BattleAnimations, Terrainbonuses,
} from './options';
...
export default function FE8ClassEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x807110;
  const size = 84;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE8 Class Editor by SpyroDi"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE8ClassEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE8ClassEditorEntries}
        />
      </div>
      <InputHex
        type={DataType.U16}
      ...
      <InputDropbox
        isHex
        type={DataType.U32}
        options={Terrainbonuses}
        view={view}
        name="Terrain defence bonus"
        offset={68}
      />
      <InputDropbox
        isHex
