...
import {
  FE8Unknown6UnitEditorEntries, CharacterList, ClassHexList, StartingLevel,
...
export default function FE8Unknown6UnitEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x8D1D10;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Unknown6 Unit Editor"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE8Unknown6UnitEditorEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE8Unknown6UnitEditorEntries}
        />
      </div>
      <InputDropbox
        isHex
