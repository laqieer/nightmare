...
import {
  BattleAnimationPointers, ItemList, WeaponAnimationType, AnimationList,
} from './options';
...
export default function FE8CustomBattleAnimationEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xE60000;
  const size = 16;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE8 Custom Battle Animation Editor by SpyroDi (please consult Custom Animations.txt)"
        type="info"
      />
      <div key={id}>
        <InputSelect
