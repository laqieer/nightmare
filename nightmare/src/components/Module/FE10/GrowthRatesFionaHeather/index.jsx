...
export default function FE10GrowthRatesFionaHeather() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x1219;
  const size = 88;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE10 Growth Rates-Fiona, Heather"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE10GrowthRates22.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE10GrowthRates22}
        />
      </div>
      <InputDec
        type={DataType.S8}
