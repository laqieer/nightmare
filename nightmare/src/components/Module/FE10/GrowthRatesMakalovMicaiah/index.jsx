...
export default function FE10GrowthRatesMakalovMicaiah() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x140d;
  const size = 92;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE10 Growth Rates-Makalov, Micaiah"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE10GrowthRates24.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE10GrowthRates24}
        />
      </div>
      <InputDec
        type={DataType.S8}
