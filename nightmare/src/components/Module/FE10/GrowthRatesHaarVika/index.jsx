...
export default function FE10GrowthRatesHaarVika() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x10c5;
  const size = 84;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE10 Growth Rates-Haar, Vika"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE10GrowthRates20.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE10GrowthRates20}
        />
      </div>
      <InputDec
        type={DataType.S8}
