...
export default function FE10GrowthRatesNepheneeNolan() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x1019;
  const size = 88;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE10 Growth Rates-Nephenee, Nolan"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE10GrowthRates19.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE10GrowthRates19}
        />
      </div>
      <InputDec
        type={DataType.S8}
