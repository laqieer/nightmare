...
export default function FE10GrowthratesLevail() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x1b9f5;
  const size = 80;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE10 Growth rates-Levail"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE10GrowthRates32.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE10GrowthRates32}
        />
      </div>
      <InputDec
        type={DataType.U8}
