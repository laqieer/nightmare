...
export default function FE10GrowthratesMiaKurthAimee() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x1aa1;
  const size = 80;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE10 Growth rates-Mia, Kurth, Aimee"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE10GrowthRates31.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE10GrowthRates31}
        />
      </div>
      <InputDec
        type={DataType.S8}
