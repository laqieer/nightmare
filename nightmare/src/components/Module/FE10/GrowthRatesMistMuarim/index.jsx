...
export default function FE10GrowthRatesMistMuarim() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x14c1;
  const size = 80;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE10 Growth Rates-Mist, Muarim"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE10GrowthRates25.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE10GrowthRates25}
        />
      </div>
      <InputDec
        type={DataType.S8}
