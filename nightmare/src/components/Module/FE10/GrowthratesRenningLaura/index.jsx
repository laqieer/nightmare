...
export default function FE10GrowthratesRenningLaura() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x19fd;
  const size = 80;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE10 Growth rates-Renning, Laura"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE10GrowthRates30.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE10GrowthRates30}
        />
      </div>
      <InputDec
        type={DataType.S8}
