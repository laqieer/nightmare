...
export default function FE10GrowthratesLeonardoLethe() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x1959;
  const size = 80;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE10 Growth rates-Leonardo, Lethe"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE10GrowthRates29.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE10GrowthRates29}
        />
      </div>
      <InputDec
        type={DataType.S8}
