...
import {
  FE42ndGenStaticCharacterEditor3, FE4CharacterType, FE4Classes, FE4CharacterNumbers,
...
export default function FE42ndGenerationStaticCharacterEditor3() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x3BAA1;
  const size = 38;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE4 2nd Generation Static Character Editor 3"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE42ndGenStaticCharacterEditor3.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE42ndGenStaticCharacterEditor3}
        />
      </div>
      <InputDropbox
        isHex
