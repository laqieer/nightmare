...
import {
  FE4WeaponEditor2, FE4WeaponNames, FE4WeaponClass, FE4WeaponBonus,
...
export default function FE4StaffEditor2() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x3F562;
  const size = 19;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE4 Weapon/Staff Editor 2"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE4WeaponEditor2.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE4WeaponEditor2}
        />
      </div>
      <InputDropbox
        isHex
