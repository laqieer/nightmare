...
import {
  FE4EnemyEditor17Entries, FE4CharacterType, FE4CharacterNumbers, FE4CharacterNameNumbers,
...
export default function FE4EnemyEditor17() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x3CCCE;
  const size = 13;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE4 Enemy Editor 17"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE4EnemyEditor17Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE4EnemyEditor17Entries}
        />
      </div>
      <InputDropbox
        isHex
