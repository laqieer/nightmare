...
import {
  FE4HolyCharacterEditor4Entries, FE4CharacterType, FE4Classes, FE4CharacterNumbers,
...
export default function FE4HolyCharacterEditor4() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x3C580;
  const size = 323;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE4 Holy Character Editor 4"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE4HolyCharacterEditor4Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE4HolyCharacterEditor4Entries}
        />
      </div>
      <InputDropbox
        isHex
