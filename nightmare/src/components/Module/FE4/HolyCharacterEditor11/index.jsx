...
import {
  FE4HolyCharacterEditor11Entries, FE4CharacterType, FE4Classes, FE4CharacterNumbers,
...
export default function FE4HolyCharacterEditor11() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x3CD90;
  const size = 90;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE4 Holy Character Editor 11"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE4HolyCharacterEditor11Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE4HolyCharacterEditor11Entries}
        />
      </div>
      <InputDropbox
        isHex
