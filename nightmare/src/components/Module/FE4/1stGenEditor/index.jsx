...
import {
  FE41stGenCharacterEditor, FE4CharacterType, FE4Classes, FE4CharacterNumbers,
...
export default function FE41stGenEditor() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x3B467;
  const size = 38;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE4 1st Gen Editor"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE41stGenCharacterEditor.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE41stGenCharacterEditor}
        />
      </div>
      <InputDropbox
        isHex
