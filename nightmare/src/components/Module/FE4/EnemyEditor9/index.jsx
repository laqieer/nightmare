...
import {
  FE4EnemyEditor9Entries, FE4CharacterType, FE4CharacterNumbers, FE4CharacterNameNumbers,
...
export default function FE4EnemyEditor9() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x3C6E9;
  const size = 13;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE4 Enemy Editor 9"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE4EnemyEditor9Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE4EnemyEditor9Entries}
        />
      </div>
      <InputDropbox
        isHex
