...
import {
  FE42ndGenStaticCharacterEditor2, FE4CharacterType, FE4Classes, FE4CharacterNumbers,
  ...
  FE4HolyBlood4, FE4PlayerEquipment,
} from './options';
...
export default function FE42ndGenerationStaticCharacterEditor2() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x3B881;
  const size = 38;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE4 2nd Generation Static Character Editor 2"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE42ndGenStaticCharacterEditor2.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE42ndGenStaticCharacterEditor2}
        />
      </div>
      <InputDropbox
        isHex
