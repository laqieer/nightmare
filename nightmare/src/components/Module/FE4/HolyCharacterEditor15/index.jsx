...
import {
  FE4HolyCharacterEditor15Entries, FE4CharacterType, FE4Classes, FE4CharacterNumbers,
...
export default function FE4HolyCharacterEditor15() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x3D20D;
  const size = 38;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE4 Holy Character Editor 15"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE4HolyCharacterEditor15Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE4HolyCharacterEditor15Entries}
        />
      </div>
      <InputDropbox
        isHex
