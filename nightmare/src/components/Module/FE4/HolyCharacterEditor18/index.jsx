...
import {
  FE4HolyCharacterEditor18Entries, FE4CharacterType, FE4Classes, FE4CharacterNumbers,
...
export default function FE4HolyCharacterEditor18() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x3D366;
  const size = 38;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE4 Holy Character Editor 18"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE4HolyCharacterEditor18Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE4HolyCharacterEditor18Entries}
        />
      </div>
      <InputDropbox
        isHex
