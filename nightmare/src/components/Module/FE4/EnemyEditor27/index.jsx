...
import {
  FE4EnemyEditor27Entries, FE4CharacterType, FE4CharacterNumbers, FE4CharacterNameNumbers,
...
export default function FE4EnemyEditor27() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x3D2D9;
  const size = 13;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="FE4 Enemy Editor 27"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE4EnemyEditor27Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE4EnemyEditor27Entries}
        />
      </div>
      <InputDropbox
        isHex
