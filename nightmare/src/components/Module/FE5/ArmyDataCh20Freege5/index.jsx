...
import {
  FE5ArmyDataCh20Freege5Entries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh20Freege5() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x189832;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch20 Freege5"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh20Freege5Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh20Freege5Entries}
        />
      </div>
      <InputDropbox
        isHex
