...
import {
  FE5Sety4GEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Sety4G() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18EDC6;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch4x Asvel/Sety4G"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5Sety4GEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5Sety4GEntries}
        />
      </div>
      <InputDropbox
        isHex
