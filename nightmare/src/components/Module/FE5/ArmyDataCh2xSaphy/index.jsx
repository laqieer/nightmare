...
import {
  FE5ArmyDataCh2xSaphyEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh2xSaphy() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18FA19;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch2x Saphy"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh2xSaphyEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh2xSaphyEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
