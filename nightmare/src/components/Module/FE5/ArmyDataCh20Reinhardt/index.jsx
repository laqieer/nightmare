...
import {
  FE5ArmyDataCh20ReinhardtEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh20Reinhardt() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x189A0C;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch20 Reinhardt"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh20ReinhardtEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh20ReinhardtEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
