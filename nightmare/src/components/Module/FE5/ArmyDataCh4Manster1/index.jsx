...
import {
  FE5ArmyDataCh4Manster1Entries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh4Manster1() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18F22B;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch4 Manster1"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh4Manster1Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh4Manster1Entries}
        />
      </div>
      <InputDropbox
        isHex
