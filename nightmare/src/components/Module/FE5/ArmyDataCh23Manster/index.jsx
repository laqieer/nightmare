...
import {
  FE5ArmyDataCh23MansterEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh23Manster() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x1883C2;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch23 Manster"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh23MansterEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh23MansterEntries}
        />
      </div>
      <InputDropbox
        isHex
