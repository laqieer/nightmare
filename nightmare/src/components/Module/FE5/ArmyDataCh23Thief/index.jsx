...
import {
  FE5ArmyDataCh23ThiefEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh23Thief() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x1883A8;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch23 Thief"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh23ThiefEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh23ThiefEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
