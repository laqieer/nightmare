...
import {
  FE5ArmyDataCh8Bandit2Entries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh8Bandit2() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18DF68;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch8 Bandit2"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh8Bandit2Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh8Bandit2Entries}
        />
      </div>
      <InputDropbox
        isHex
