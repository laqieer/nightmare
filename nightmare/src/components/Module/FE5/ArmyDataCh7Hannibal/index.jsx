...
import {
  FE5ArmyDataCh7HannibalEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh7Hannibal() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18E173;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch7 Hannibal"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh7HannibalEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh7HannibalEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
