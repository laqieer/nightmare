...
import {
  FE5ArmyDataCh21SemitolEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh21Semitol() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x1892B5;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch21 Semitol"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh21SemitolEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh21SemitolEntries}
        />
      </div>
      <InputDropbox
        isHex
