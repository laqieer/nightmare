...
import {
  FE5SetyEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Sety() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x1883EC;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch23 Cyas/Sety"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5SetyEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5SetyEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
