...
import {
  FE5ArmyDataCh7IzenauEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh7Izenau() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18E09B;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch7 Izenau"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh7IzenauEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh7IzenauEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
