...
import {
  FE5ArmyDataCh15Bandit4Entries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh15Bandit4() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18B85F;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch15 Bandit4"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh15Bandit4Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh15Bandit4Entries}
        />
      </div>
      <InputHex
        type={DataType.U8}
