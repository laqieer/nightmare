...
import {
  FE5TinaEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Tina() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18C5F9;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch12x Trewd/Pahn/Tina"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5TinaEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5TinaEntries}
        />
      </div>
      <InputDropbox
        isHex
