...
import {
  FE5ArmyDataCh21RoputoEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh21Roputo() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18930D;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch21 Roputo"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh21RoputoEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh21RoputoEntries}
        />
      </div>
      <InputDropbox
        isHex
