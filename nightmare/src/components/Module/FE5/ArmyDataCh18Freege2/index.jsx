...
import {
  FE5ArmyDataCh18Freege2Entries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh18Freege2() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18A2EE;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch18 Freege2"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh18Freege2Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh18Freege2Entries}
        />
      </div>
      <InputDropbox
        isHex
