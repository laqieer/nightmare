...
import {
  FE5OruenEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Oruen() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18D69D;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch10 Fred/Oruen"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5OruenEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5OruenEntries}
        />
      </div>
      <InputDropbox
        isHex
