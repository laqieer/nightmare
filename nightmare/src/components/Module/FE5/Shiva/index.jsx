...
import {
  FE5ShivaEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Shiva() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18F86F;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch2x Rifis/Shiva"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ShivaEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ShivaEntries}
        />
      </div>
      <InputDropbox
        isHex
