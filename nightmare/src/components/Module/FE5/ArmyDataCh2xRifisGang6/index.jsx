...
import {
  FE5ArmyDataCh2xRifisGang6Entries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh2xRifisGang6() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18F9EF;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch2x Rifis Gang6"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh2xRifisGang6Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh2xRifisGang6Entries}
        />
      </div>
      <InputDropbox
        isHex
