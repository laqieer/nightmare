...
import {
  FE5ArmyDataCh6HicksEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh6Hicks() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18E6AE;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch6 Hicks"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh6HicksEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh6HicksEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
