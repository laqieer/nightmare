...
import {
  FE5ConomoreEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Conomore() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x189CF7;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch19 Amalda/Conomore"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ConomoreEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ConomoreEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
