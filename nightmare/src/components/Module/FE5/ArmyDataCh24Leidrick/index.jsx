...
import {
  FE5ArmyDataCh24LeidrickEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh24Leidrick() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xCF4A6;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch24 Leidrick"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh24LeidrickEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh24LeidrickEntries}
        />
      </div>
      <InputDropbox
        isHex
