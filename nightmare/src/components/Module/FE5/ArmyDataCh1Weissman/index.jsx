...
import {
  FE5ArmyDataCh1WeissmanEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh1Weissman() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18FDE1;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch1 Weissman"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh1WeissmanEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh1WeissmanEntries}
        />
      </div>
      <InputDropbox
        isHex
