...
import {
  FE5LaraEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Lara() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18F197;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch4 Sety4G/Asvel/Brighton/Machyua/Lara"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5LaraEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5LaraEntries}
        />
      </div>
      <InputDropbox
        isHex
