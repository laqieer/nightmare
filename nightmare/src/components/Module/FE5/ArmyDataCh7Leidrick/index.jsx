...
import {
  FE5ArmyDataCh7LeidrickEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh7Leidrick() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18E243;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch7 Leidrick"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh7LeidrickEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh7LeidrickEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
