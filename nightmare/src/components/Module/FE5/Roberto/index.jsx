...
import {
  FE5RobertoEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Roberto() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18D9AE;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch9 Alba/Roberto"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5RobertoEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5RobertoEntries}
        />
      </div>
      <InputDropbox
        isHex
