...
import {
  FE5ArmyDataCh16ANikolafEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh16ANikolaf() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18B51A;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch16A Nikolaf"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh16ANikolafEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh16ANikolafEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
