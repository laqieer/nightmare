...
import {
  FE5ArmyDataCh24Roputo2Entries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh24Roputo2() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xCF8AE;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch24 Roputo2"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh24Roputo2Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh24Roputo2Entries}
        />
      </div>
      <InputDropbox
        isHex
