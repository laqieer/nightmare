...
import {
  FE5ArmyDataCh21xFreege3Entries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh21xFreege3() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x188D82;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch21x Freege3"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh21xFreege3Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh21xFreege3Entries}
        />
      </div>
      <InputHex
        type={DataType.U8}
