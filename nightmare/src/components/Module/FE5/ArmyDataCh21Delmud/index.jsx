...
import {
  FE5ArmyDataCh21DelmudEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh21Delmud() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x189337;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch21 Delmud"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh21DelmudEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh21DelmudEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
