...
import {
  FE5ArmyDataCh17BPalmanEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh17BPalman() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18A4AB;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch17B Palman"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh17BPalmanEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh17BPalmanEntries}
        />
      </div>
      <InputDropbox
        isHex
