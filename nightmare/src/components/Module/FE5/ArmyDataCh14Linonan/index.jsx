...
import {
  FE5ArmyDataCh14LinonanEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh14Linonan() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18B97A;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch14 Linonan"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh14LinonanEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh14LinonanEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
