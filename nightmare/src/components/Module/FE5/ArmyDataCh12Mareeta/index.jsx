...
import {
  FE5ArmyDataCh12MareetaEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh12Mareeta() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18CBA8;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch12 Mareeta"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh12MareetaEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh12MareetaEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
