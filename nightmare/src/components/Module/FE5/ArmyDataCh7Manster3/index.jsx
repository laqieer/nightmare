...
import {
  FE5ArmyDataCh7Manster3Entries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh7Manster3() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18E135;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch7 Manster3"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh7Manster3Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh7Manster3Entries}
        />
      </div>
      <InputDropbox
        isHex
