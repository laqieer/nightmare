...
import {
  FE5XavierEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Xavier() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x189EEA;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch18 Gustav/Xavier"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5XavierEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5XavierEntries}
        />
      </div>
      <InputDropbox
        isHex
