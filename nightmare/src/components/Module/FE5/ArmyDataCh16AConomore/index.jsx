...
import {
  FE5ArmyDataCh16AConomoreEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh16AConomore() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18B36C;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch16A Conomore"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh16AConomoreEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh16AConomoreEntries}
        />
      </div>
      <InputDropbox
        isHex
