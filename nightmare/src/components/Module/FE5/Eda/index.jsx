...
import {
  FE5EdaEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Eda() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18C113;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch14 Arione/Torborant/Homeros/Dean/Eda"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5EdaEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5EdaEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
