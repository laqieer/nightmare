...
import {
  FE5TorabantEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Torabant() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18D9DA;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch9 Altenna/Torabant"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5TorabantEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5TorabantEntries}
        />
      </div>
      <InputDropbox
        isHex
