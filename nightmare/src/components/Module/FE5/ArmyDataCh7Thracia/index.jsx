...
import {
  FE5ArmyDataCh7ThraciaEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh7Thracia() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18E18D;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch7 Thracia"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh7ThraciaEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh7ThraciaEntries}
        />
      </div>
      <InputDropbox
        isHex
