...
import {
  FE5ArmyDataCh11xFredEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh11xFred() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18D0D3;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch11x Fred"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh11xFredEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh11xFredEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
