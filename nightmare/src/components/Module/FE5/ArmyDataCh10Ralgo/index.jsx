...
import {
  FE5ArmyDataCh10RalgoEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh10Ralgo() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18D3EF;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch10 Ralgo"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh10RalgoEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh10RalgoEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
