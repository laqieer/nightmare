...
import {
  FE5ArmyDataCh19BanditEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh19Bandit() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x189D29;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch19 Bandit"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh19BanditEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh19BanditEntries}
        />
      </div>
      <InputDropbox
        isHex
