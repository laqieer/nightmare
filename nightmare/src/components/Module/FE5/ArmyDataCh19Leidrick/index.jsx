...
import {
  FE5ArmyDataCh19LeidrickEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh19Leidrick() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x189EA9;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch19 Leidrick"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh19LeidrickEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh19LeidrickEntries}
        />
      </div>
      <InputDropbox
        isHex
