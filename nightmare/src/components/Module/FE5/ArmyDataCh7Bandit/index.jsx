...
import {
  FE5ArmyDataCh7BanditEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh7Bandit() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18E25D;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch7 Bandit"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh7BanditEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh7BanditEntries}
        />
      </div>
      <InputDropbox
        isHex
