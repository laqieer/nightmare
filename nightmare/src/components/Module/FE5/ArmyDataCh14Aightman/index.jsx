...
import {
  FE5ArmyDataCh14AightmanEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh14Aightman() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18B900;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch14 Aightman"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh14AightmanEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh14AightmanEntries}
        />
      </div>
      <InputDropbox
        isHex
