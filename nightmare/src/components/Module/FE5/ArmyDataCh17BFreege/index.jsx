...
import {
  FE5ArmyDataCh17BFreegeEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh17BFreege() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18A83B;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch17B Freege"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh17BFreegeEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh17BFreegeEntries}
        />
      </div>
      <InputDropbox
        isHex
