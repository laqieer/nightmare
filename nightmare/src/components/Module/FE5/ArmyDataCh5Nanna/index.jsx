...
import {
  FE5ArmyDataCh5NannaEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh5Nanna() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18E8B1;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch5 Nanna"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh5NannaEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh5NannaEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
