...
import {
  FE5MishaEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Misha() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18AB0B;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch17A Nokolaf/Misha"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5MishaEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5MishaEntries}
        />
      </div>
      <InputDropbox
        isHex
