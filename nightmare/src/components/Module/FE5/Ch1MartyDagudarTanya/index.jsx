...
import {
  FE5Ch1MartyDagudarTanyaEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Ch1MartyDagudarTanya() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18FDA1;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Ch1 Marty, Dagudar, Tanya"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5Ch1MartyDagudarTanyaEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5Ch1MartyDagudarTanyaEntries}
        />
      </div>
      <InputDropbox
        isHex
