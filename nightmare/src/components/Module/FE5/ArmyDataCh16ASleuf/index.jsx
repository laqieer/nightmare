...
import {
  FE5ArmyDataCh16ASleufEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh16ASleuf() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18B45E;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch16A Sleuf"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh16ASleufEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh16ASleufEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
