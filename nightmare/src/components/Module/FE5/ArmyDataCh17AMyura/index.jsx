...
import {
  FE5ArmyDataCh17AMyuraEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh17AMyura() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18AEA7;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch17A Myura"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh17AMyuraEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh17AMyuraEntries}
        />
      </div>
      <InputDropbox
        isHex
