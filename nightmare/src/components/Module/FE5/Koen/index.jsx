...
import {
  FE5KoenEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Koen() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x1889ED;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch22 Cyas/Koen"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5KoenEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5KoenEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
