...
import {
  FE5IshtarEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Ishtar() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18AF71;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch17A Cyas/Sara/Reinhardt/Ishtar"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5IshtarEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5IshtarEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
