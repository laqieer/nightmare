...
import {
  FE5ArmyDataCh11xNPCsEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh11xNPCs() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18D095;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch11x NPCs"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh11xNPCsEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh11xNPCsEntries}
        />
      </div>
      <InputDropbox
        isHex
