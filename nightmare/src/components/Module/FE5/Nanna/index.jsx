...
import {
  FE5NannaEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Nanna() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18FF31;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch1 Mareeta/Nanna"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5NannaEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5NannaEntries}
        />
      </div>
      <InputDropbox
        isHex
