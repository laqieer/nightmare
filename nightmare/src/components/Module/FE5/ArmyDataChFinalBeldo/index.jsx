...
import {
  FE5ArmyDataChFinalBeldoEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataChFinalBeldo() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xCFCF2;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - ChFinal Beldo"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataChFinalBeldoEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataChFinalBeldoEntries}
        />
      </div>
      <InputDropbox
        isHex
