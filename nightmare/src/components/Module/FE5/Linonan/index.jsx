...
import {
  FE5LinonanEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Linonan() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18B8A3;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch14x Sleuf/Linonan"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5LinonanEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5LinonanEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
