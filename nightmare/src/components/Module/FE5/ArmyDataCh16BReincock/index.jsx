...
import {
  FE5ArmyDataCh16BReincockEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh16BReincock() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18A8FC;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch16B Reincock"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh16BReincockEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh16BReincockEntries}
        />
      </div>
      <InputDropbox
        isHex
