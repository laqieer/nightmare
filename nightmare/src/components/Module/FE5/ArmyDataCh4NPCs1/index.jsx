...
import {
  FE5ArmyDataCh4NPCs1Entries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh4NPCs1() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18F43F;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch4 NPCs1"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh4NPCs1Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh4NPCs1Entries}
        />
      </div>
      <InputDropbox
        isHex
