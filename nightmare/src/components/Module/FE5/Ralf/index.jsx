...
import {
  FE5RalfEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Ralf() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18B879;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch15 Amalda/Ralf"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5RalfEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5RalfEntries}
        />
      </div>
      <InputDropbox
        isHex
