...
import {
  FE5ArmyDataCh21PhlausEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh21Phlaus() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x188F51;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch21 Phlaus"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh21PhlausEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh21PhlausEntries}
        />
      </div>
      <InputDropbox
        isHex
