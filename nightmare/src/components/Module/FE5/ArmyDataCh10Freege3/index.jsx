...
import {
  FE5ArmyDataCh10Freege3Entries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh10Freege3() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18D653;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch10 Freege3"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh10Freege3Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh10Freege3Entries}
        />
      </div>
      <InputHex
        type={DataType.U8}
