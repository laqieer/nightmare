...
import {
  FE5ArmyDataCh15Bandit3Entries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh15Bandit3() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18B821;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch15 Bandit3"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh15Bandit3Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh15Bandit3Entries}
        />
      </div>
      <InputDropbox
        isHex
