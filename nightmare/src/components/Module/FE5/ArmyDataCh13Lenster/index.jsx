...
import {
  FE5ArmyDataCh13LensterEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh13Lenster() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18C4E0;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch13 Lenster"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh13LensterEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh13LensterEntries}
        />
      </div>
      <InputDropbox
        isHex
