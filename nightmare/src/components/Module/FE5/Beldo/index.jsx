...
import {
  FE5BeldoEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Beldo() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xCF8D8;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch24 Galzus/Beldo"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5BeldoEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5BeldoEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
