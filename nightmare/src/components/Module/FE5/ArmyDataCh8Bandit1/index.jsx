...
import {
  FE5ArmyDataCh8Bandit1Entries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh8Bandit1() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18DF06;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch8 Bandit1"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh8Bandit1Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh8Bandit1Entries}
        />
      </div>
      <InputHex
        type={DataType.U8}
