...
import {
  FE5KarinEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Karin() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18F1FF;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch4 Felgus/Karin"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5KarinEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5KarinEntries}
        />
      </div>
      <InputDropbox
        isHex
