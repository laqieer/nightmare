...
import {
  FE5ArmyDataCh21FreegeEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh21Freege() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x189239;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch21 Freege"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh21FreegeEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh21FreegeEntries}
        />
      </div>
      <InputDropbox
        isHex
