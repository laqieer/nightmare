...
import {
  FE5ArmyDataCh11FredEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh11Fred() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18D330;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch11 Fred"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh11FredEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh11FredEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
