...
import {
  FE5ManfroyEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Manfroy() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18F4BF;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch4 Ishtar/Yurius/Beldo/Manfroy"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ManfroyEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ManfroyEntries}
        />
      </div>
      <InputDropbox
        isHex
