...
import {
  FE5LeidrickEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Leidrick() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x189E8F;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch19 Ishtar/Leidrick"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5LeidrickEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5LeidrickEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
