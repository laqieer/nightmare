...
import {
  FE5ArmyDataCh15ZailEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh15Zail() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18B5B5;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch15 Zail"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh15ZailEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh15ZailEntries}
        />
      </div>
      <InputDropbox
        isHex
