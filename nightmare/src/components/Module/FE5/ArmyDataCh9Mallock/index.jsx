...
import {
  FE5ArmyDataCh9MallockEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh9Mallock() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18D708;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch9 Mallock"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh9MallockEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh9MallockEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
