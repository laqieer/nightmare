...
import {
  FE5CyasEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Cyas() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18E7E8;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch6 Galzus/Mareeta/Cyas"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5CyasEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5CyasEntries}
        />
      </div>
      <InputDropbox
        isHex
