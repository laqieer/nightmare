...
import {
  FE5ArmyDataCh2RifisGang8Entries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh2RifisGang8() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18FC92;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch2 Rifis Gang8"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh2RifisGang8Entries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh2RifisGang8Entries}
        />
      </div>
      <InputDropbox
        isHex
