...
import {
  FE5ArmyDataCh17BAmaldaEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh17BAmalda() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18A821;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch17B Amalda"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh17BAmaldaEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh17BAmaldaEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
