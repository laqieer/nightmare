...
import {
  FE5GalzusEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Galzus() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x18EBD9;
  const size = 24;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch5 Mareeta/Galzus"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5GalzusEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5GalzusEntries}
        />
      </div>
      <InputHex
        type={DataType.U8}
