...
import {
  FE5ArmyDataCh24xSniperEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5ArmyDataCh24xSniper() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0xCFBD9;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch24x Sniper"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5ArmyDataCh24xSniperEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5ArmyDataCh24xSniperEntries}
        />
      </div>
      <InputDropbox
        isHex
