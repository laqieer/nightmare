...
import {
  FE5FerdanEntries, FE5CharacterList, FE5ItemData,
} from './options';
...
export default function FE5Ferdan() {
  const [buffer] = useOutletContext();
  const [searchParams, setSearchParams] = useSearchParams();

  const id = searchParams.get('id');
  const address = 0x1885DA;
  const size = 20;
  let view = null;
  ...
  return (
    ...
    >
      <Alert
        message="Army Data - Ch23 Alphane/Ferdan"
        type="info"
      />
      <div key={id}>
        <InputSelect
          disabled={view == null}
          defaultValue={FE5FerdanEntries.find(
            (entry) => entry.value.toString() === id,
          ) ?? id}
          onSelect={(value) => setSearchParams({ id: value })}
          options={FE5FerdanEntries}
        />
      </div>
      <InputDropbox
        isHex
